...
def test_admin_summary_requires_admin_role():
    client = make_client()
    with (
        patch("app.api.routes.verify_supabase_token", return_value=principal()),
        patch(
            "app.api.routes.admin_service.resolve_scope",
...
def test_admin_summary_passes_organization_filter_for_org_admin():
    ...
    with (
        patch("app.api.routes.verify_supabase_token", return_value=principal()),
        patch("app.api.routes.admin_service.resolve_scope", return_value=org_scope),
        patch(
            "app.api.routes.admin_service.get_summary", return_value=summary
        ) as get_summary,
    ):
        response = client.get(
...
def test_admin_organizations_returns_visible_companies():
    ...
    with (
        ...
        patch("app.api.routes.admin_service.resolve_scope", return_value=org_scope),
        patch(
            "app.api.routes.admin_service.list_organizations",
