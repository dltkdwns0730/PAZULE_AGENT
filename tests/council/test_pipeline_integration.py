...
def _print_integration_step(node_name: str, state: dict):
    ...
    if node_name == "validator":
        ...
        print(f"  - Model Votes: {len(artifacts.get('model_votes', []))} responses collected.")
    elif node_name == "aggregator":
        ensemble = artifacts.get('ensemble_result', {})
        print(f"  - Aggregated Score: {ensemble.get('merged_score')} / Threshold: {ensemble.get('threshold')}")
        ...
        print(f"  - Judgment Success: {judgment.get('success')} (Reason: {judgment.get('reason')})")
...
class TestPipelineIntegration:
    def test_full_pipeline_success(self, mocker):
        ...
        mocker.patch("app.council.nodes.mission_session_service.hash_file", return_value="fake_hash_int")
        ...
        mock_invoke.return_value = {"model": "mock_model", "score": 0.95, "label": "match"}
    ...
    def test_full_pipeline_gate_blocked(self, mocker):
        ...
        mocker.patch("app.council.nodes.mission_session_service.hash_file", return_value="fake_hash_int")
        mocker.patch("app.council.nodes.mission_session_service.is_duplicate_hash_for_user", return_value=True)
