...
def run_test_pipeline():
    try:
        ...
        image_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "docs", "assets", "pipeline_architecture.png")
        user_id = "test-user"
        site_id = "test-site"
            ...
            for node_name, state in s.items():
                ...
                if node_name == "validator":
                    ...
                    for idx, vote in enumerate(artifacts.get('model_votes', [])):
                        print(f"    [{idx}] Model: {vote.get('model_name', 'N/A')}, Score: {vote.get('score', 0):.2f}, Hint: {vote.get('hint', '')[:50]}...")
                    ...
                    print(f" - Deliberation Output: {artifacts.get('deliberation_output')}")
                elif node_name == "judge":
                    print(f" - Judgment: {artifacts.get('judgment')}")
                elif node_name == "policy":
                    print(f" - Policy Result: {artifacts.get('policy_result')}")
                elif node_name == "responder":
                    print(f" - Final Response Data: {state.get('final_response', {}).get('data')}")
