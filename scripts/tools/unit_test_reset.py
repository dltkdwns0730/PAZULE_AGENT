...
def test_reset_logic():
    ...
    s1 = mission_session_service.create_session(user_id, "site", "location", "ans1", "hint1")
    ...
    s2 = mission_session_service.create_session(user_id, "site", "atmosphere", "ans2", "hint2")
    ...
    print(f"After Reset: attempts={stats_after['total_attempts']}, coupons={len(coupons_after)}")
    ...
    is_completed = mission_session_service.is_mission_completed_today(user_id, "location")
    print(f"Today's mission completed: {is_completed}")
    assert is_completed == True, "Today's mission should remain completed"
