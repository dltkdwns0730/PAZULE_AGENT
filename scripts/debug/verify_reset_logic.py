...
def setup_mock_data():
    ...
    coupons = {
        "coupons": [
            {"code": "TODAY-COUPON", "user_id": "guest", "mission_id": "today-success"},
            {
                "code": "OLD-COUPON",
                "user_id": "guest",
                "mission_id": "yesterday-success",
...
def verify_reset():
    ...
    stats = res.json()
    print(
        f"Before reset: total_attempts={stats['total_attempts']}, total_coupons={stats['total_coupons']}"
    )
    ...
    stats = res.json()
    print(
        f"After reset stats: total_attempts={stats['total_attempts']}, total_coupons={stats['total_coupons']}"
    )
    ...
    assert hint_data.get("completed"), "Today's mission should still show as completed"
