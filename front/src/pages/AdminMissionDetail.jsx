...
export default function AdminMissionDetail() {
  ...
  useEffect(() => {
    ...
    adminApi
      ...
      .then(setMission)
      .catch((exc) => setError(exc instanceof Error ? exc.message : '미션 상세 정보를 불러오지 못했습니다.'));
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
              <div>
                <p className="text-sm font-bold text-slate-500">미션 ID</p>
            ...
            <dl className="mt-5 grid gap-4 md:grid-cols-2">
              <div>
                <dt className="text-xs font-bold uppercase text-slate-400">사용자</dt>
                <dd className="mt-1 text-sm font-semibold">{mission.user_id}</dd>
              </div>
              <div>
                <dt className="text-xs font-bold uppercase text-slate-400">유형</dt>
                <dd className="mt-1 text-sm font-semibold">{mission.mission_type}</dd>
              </div>
              <div>
                <dt className="text-xs font-bold uppercase text-slate-400">정답</dt>
                <dd className="mt-1 text-sm font-semibold">{mission.answer}</dd>
              </div>
              <div>
                <dt className="text-xs font-bold uppercase text-slate-400">쿠폰</dt>
                <dd className="mt-1 text-sm font-semibold">{mission.coupon_code || '-'}</dd>
              </div>
            </dl>
            <div className="mt-6 rounded-lg bg-slate-50 p-4">
              <p className="text-xs font-bold uppercase text-slate-400">힌트</p>
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-500">최근 판정</p>
                  <p className="text-xl font-black">{success ? '성공' : '성공 아님'}</p>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
              <h3 className="border-b border-slate-100 px-5 py-4 text-base font-black">제출 이력</h3>
              <div className="divide-y divide-slate-100">
                ...
                {(mission.submissions || []).length === 0 && (
                  <p className="px-5 py-6 text-sm font-semibold text-slate-500">기록된 제출 이력이 없습니다.</p>
                )}
              </div>
            </div>
          </div>
